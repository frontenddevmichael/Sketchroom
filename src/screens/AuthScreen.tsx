...
function GoogleGlyph({ size }: { size: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.48 24 48 24z" />
    </svg>
  );
}
...
export function AuthScreen() {
  ...
  const run = async (provider: string, params: Record<string, string>) => {
    ...
    try {
      ...
      if (!signingIn) {
        ...
        else if (params.flow === 'reset-verification') setStep({ kind: 'resetDone' });
  ...
  const onPassword = (e: FormEvent<HTMLFormElement>) => {
    ...
    void run('password', {
      ...
      ...(step.kind === 'signup' ? { name: String(fd.get('name') ?? '').trim() } : {}),
    });
  };
  ...
  const onVerify = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const email = step.kind === 'verify' ? step.email : String(fd.get('email') ?? '').trim();
    void run('password', {
  ...
  const onReset = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const email = step.kind === 'reset' ? step.email : String(fd.get('email') ?? '').trim();
    void run('password', {
  ...
  const passwordTitle = step.kind === 'signup' ? 'Create your account' : 'Welcome back';
