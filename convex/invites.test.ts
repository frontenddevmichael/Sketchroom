...
test("revokeInvite expires a pending invite so it can no longer be accepted", async () => {
  ...
  await expect(
    strangerT.mutation(api.invites.revokeInvite, { inviteId: invite._id })
  ).rejects.toThrow("Insufficient permissions");
  const ok = await aliceT.mutation(api.invites.revokeInvite, { inviteId: invite._id });
  expect(ok).toBe(true);
  await expect(bobT.mutation(api.invites.acceptInvite, { token })).rejects.toThrow(
    "Invalid or expired"
  );
});
...
test("inviteMember stores a lowercased pending invite", async () => {
  ...
  const { inviteId, token } = await aliceT.mutation(api.invites.inviteMember, {
    roomId,
    email: " BoB@Example.com ",
    role: "editor",
  });
  expect(token.length).toBeGreaterThan(10);
  const invites = await aliceT.query(api.invites.getRoomInvites, { roomId });
...
test("inviteMember is author-only, dedupes pending invites, rejects members", async () => {
  ...
  const strangerT = t.withIdentity({ ...alice, subject: "user_stranger" });
  await expect(
    strangerT.mutation(api.invites.inviteMember, { roomId, email: "x@y.com", role: "viewer" })
  ).rejects.toThrow("Insufficient permissions");

  await aliceT.mutation(api.invites.inviteMember, { roomId, email: "x@y.com", role: "viewer" });
  await expect(
    aliceT.mutation(api.invites.inviteMember, { roomId, email: "X@Y.com", role: "viewer" })
  ).rejects.toThrow("already pending");

  // Bob is not a member yet, so he cannot invite either.
  await expect(
    bobT.mutation(api.invites.inviteMember, { roomId, email: "z@w.com", role: "viewer" })
  ).rejects.toThrow("Insufficient permissions");
});
...
test("acceptInvite adds a member with the invited role and marks it accepted", async () => {
  ...
  const { roomId: joinedId } = await bobT.mutation(api.invites.acceptInvite, { token });
  expect(joinedId).toBe(roomId);
  const members = await aliceT.query(api.invites.listMembers, { roomId });
...
test("acceptInvite rejects when email does not match", async () => {
  ...
  const { token } = await aliceT.mutation(api.invites.inviteMember, {
    roomId,
    email: "bob@example.com",
    role: "viewer",
  });
  const carolT = t.withIdentity({ ...alice, subject: "user_carol", email: "carol@example.com" });
  await expect(carolT.mutation(api.invites.acceptInvite, { token })).rejects.toThrow(
    "doesn't match"
  );
});
...
test("acceptInvite marks expired invites as expired", async () => {
  ...
  await expect(
    t.withIdentity(bob).mutation(api.invites.acceptInvite, { token })
  ).rejects.toThrow("expired");
});

test("listMembers is owner/editor only", async () => {
  const { aliceT, bobT, roomId } = await setup();
  const invite = await aliceT.mutation(api.invites.inviteMember, {
    roomId,
    email: "bob@example.com",
    role: "viewer",
  });
  await bobT.mutation(api.invites.acceptInvite, { token: invite.token });
  // Bob is a viewer, so listMembers must not expose the roster to him.
  const asBob = await bobT.query(api.invites.listMembers, { roomId });
  expect(asBob).toEqual([]);
  const asAlice = await aliceT.query(api.invites.listMembers, { roomId });
