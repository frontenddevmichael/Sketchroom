...
async function addBobAsEditor(aliceT: Setup["aliceT"], bobT: Setup["bobT"], roomId: Setup["roomId"]) {
...
test("upsertPresence stores cursor, camera, and selection; getPresence excludes self", async () => {
  ...
  await aliceT.mutation(api.features.presence.upsertPresence, {
    roomId,
    name: "Alice",
    color: "#fff",
    cursorX: 12,
  ...
  await bobT.mutation(api.features.presence.upsertPresence, {
    roomId,
    name: "Bob",
    color: "#000",
    cursorX: 1,
  ...
  const asAlice = await aliceT.query(api.features.presence.getPresence, { roomId });
  ...
  const asBob = await bobT.query(api.features.presence.getPresence, { roomId });
...
test("prunePresence deletes stale rows of others but never its own", async () => {
  const { aliceT, bobT, roomId } = await setup();
  await addBobAsEditor(aliceT, bobT, roomId);
  await aliceT.mutation(api.features.presence.upsertPresence, { roomId, name: "Alice", color: "#fff" });
  await bobT.mutation(api.features.presence.upsertPresence, { roomId, name: "Bob", color: "#000" });
  ...
  const pruned = await aliceT.mutation(api.features.presence.prunePresence, { roomId });
...
test("upsertPresence and getPresence require membership", async () => {
  ...
  await expect(
    stranger.mutation(api.features.presence.upsertPresence, { roomId, name: "X", color: "#fff" })
  ).rejects.toThrow("Not a member");
  await expect(stranger.query(api.features.presence.getPresence, { roomId })).resolves.toEqual([]);
});
