...
test("reportError logs an error row for the authed user", async () => {
  ...
  const ok = await aliceT.mutation(api.core.errors.reportError, {
    source: "react",
    message: "boom",
...
test("reportError works unauthenticated and rejects empty messages", async () => {
  const t = convexTest(schema, modules);
  const ok = await t.mutation(api.core.errors.reportError, {
    source: "react",
    message: "anon error",
  });
  expect(ok).toBe(true);
  const rejected = await t.mutation(api.core.errors.reportError, { source: "react", message: "" });
