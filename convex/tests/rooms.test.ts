...
test("createWorkspace ties the workspace to the authed user", async () => {
  ...
  const ws = await aliceT.mutation(api.features.rooms.createWorkspace, { name: "Beta" });
  expect(ws.ownerId).toBe(alice.subject);
  const workspaces = await aliceT.query(api.features.rooms.getWorkspaces);
  expect(workspaces.some((w) => w._id === ws.id && w.name === "Beta")).toBe(true);
});

test("getWorkspaces returns [] for unauthenticated users", async () => {
  const { t } = await setup();
  const workspaces = await t.query(api.features.rooms.getWorkspaces);
  expect(workspaces).toEqual([]);
});

test("createRoom adds an owner membership and returns the room", async () => {
  const { aliceT, workspaceId } = await setup();
  const room = await aliceT.mutation(api.features.rooms.createRoom, {
    workspaceId,
    name: "Design",
  });
  expect(room.ownerId).toBe(alice.subject);
  const fetched = await aliceT.query(api.features.rooms.getRoom, { roomId: room.id });
...
test("getRooms only lists rooms the user belongs to in that workspace", async () => {
  ...
  const { id: bobWsId } = await bobT.mutation(api.features.rooms.createWorkspace, { name: "Bob Inc" });
  await aliceT.mutation(api.features.rooms.createRoom, {
    workspaceId,
    name: "Shared",
  });
  await bobT.mutation(api.features.rooms.createRoom, {
    workspaceId: bobWsId,
    name: "Bob-only",
  });
  const rooms = await aliceT.query(api.features.rooms.getRooms, { workspaceId });
...
test("createRoom rejects users who do not own the workspace", async () => {
  ...
  const bobT = t.withIdentity({ ...alice, subject: "user_bob", email: "bob@example.com" });
  await expect(
    bobT.mutation(api.features.rooms.createRoom, { workspaceId, name: "Hijack" })
...
test("getRoom returns null when the user is not a member", async () => {
  const { aliceT, t, workspaceId } = await setup();
  const bobT = t.withIdentity({ ...alice, subject: "user_bob", email: "bob@example.com" });
  const room = await aliceT.mutation(api.features.rooms.createRoom, { workspaceId, name: "Solo" });
  const fetched = await bobT.query(api.features.rooms.getRoom, { roomId: room.id });
...
test("updateWorkspaceName is owner-only and rejects empty names", async () => {
  ...
  const bobT = t.withIdentity({ ...alice, subject: "user_bob", email: "bob@example.com" });
  await expect(
    bobT.mutation(api.features.rooms.updateWorkspaceName, { workspaceId, name: "Hijacked" })
  ).rejects.toThrow("Only owner can update workspace");
  await expect(
    aliceT.mutation(api.features.rooms.updateWorkspaceName, { workspaceId, name: "   " })
  ).rejects.toThrow("Name cannot be empty");
  const ok = await aliceT.mutation(api.features.rooms.updateWorkspaceName, {
    workspaceId,
    name: "Acme 2",
  });
  expect(ok).toBe(true);
...
test("deleteRoom is owner-only and cascades related rows", async () => {
  const { aliceT, t, workspaceId } = await setup();
  const bobT = t.withIdentity({ ...alice, subject: "user_bob", email: "bob@example.com" });
  const room = await aliceT.mutation(api.features.rooms.createRoom, { workspaceId, name: "Doomed" });
  await aliceT.mutation(api.features.snapshots.saveSnapshot, {
    roomId: room.id,
    canvasData: "{}",
    description: "v0",
  });
  await expect(
    bobT.mutation(api.features.rooms.deleteRoom, { roomId: room.id })
  ).rejects.toThrow("Only owner can delete");
  const ok = await aliceT.mutation(api.features.rooms.deleteRoom, { roomId: room.id });
  expect(ok).toBe(true);
  expect(await aliceT.query(api.features.rooms.getRoom, { roomId: room.id })).toBeNull();
  const snapshots = await aliceT.query(api.features.snapshots.listSnapshots, { roomId: room.id });
  expect(snapshots).toEqual([]);
});

test("deleteWorkspace removes the workspace and all its rooms", async () => {
  const { aliceT, workspaceId } = await setup();
  const room = await aliceT.mutation(api.features.rooms.createRoom, { workspaceId, name: "Child" });
  const ok = await aliceT.mutation(api.features.rooms.deleteWorkspace, { workspaceId });
  expect(ok).toBe(true);
  expect(await aliceT.query(api.features.rooms.getRoom, { roomId: room.id })).toBeNull();
  expect(await aliceT.query(api.features.rooms.getWorkspaces)).toEqual([]);
});

test("getUsage counts rooms and completed AI suggestions", async () => {
  const { aliceT, workspaceId } = await setup();
  await aliceT.mutation(api.features.rooms.createRoom, { workspaceId, name: "A" });
  await aliceT.mutation(api.features.rooms.createRoom, { workspaceId, name: "B" });
  const usage = await aliceT.query(api.features.rooms.getUsage);
...
test("getRooms attaches a member avatar summary to each card", async () => {
  ...
  for (const viewer of [carolT, daveT]) {
    const link = await aliceT.mutation(api.features.invites.createInviteLink, { roomId, role: "viewer" });
    await viewer.mutation(api.features.invites.acceptInvite, { token: link.token });
  }
...
test("updateRoomName records the server-derived last editor", async () => {
  ...
  await aliceT.mutation(api.features.rooms.updateRoomName, { roomId, name: "Design v2" });
  const room = await aliceT.query(api.features.rooms.getRoom, { roomId });
...
test("updateRoomThumbnail rejects an oversized base64 thumbnail", async () => {
  ...
  await expect(
    aliceT.mutation(api.features.rooms.updateRoomThumbnail, { roomId, thumbnailData: huge })
